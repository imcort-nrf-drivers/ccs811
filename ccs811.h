...
bool CCS811_start(int mode);                                                         // Switched CCS811 to `mode`, use constants CCS811_MODE_XXX. Returns false on I2C problems.
...
//void CCS811_set_i2cdelay(int us); // Delay before a repeated start - needed for e.g. ESP8266 because it doesn't handle I2C clock stretch correctly
//int CCS811_get_i2cdelay(void);    // Get current delay

//void CCS811_wake_init(void); // Initialize nwake pin
//void CCS811_wake_up(void);   // Wake up CCS811, i.e. pull nwake pin low
//void CCS811_wake_down(void); // CCS811 back to sleep, i.e. pull nwake pin high

//bool CCS811_i2cwrite(uint8_t regaddr, int count, const uint8_t *buf); // Writes `count` from `buf` to register at address `regaddr` in the CCS811. Returns false on I2C problems.
//bool CCS811_i2cread(uint8_t regaddr, int count, uint8_t *buf);        // Reads 'count` bytes from register at address `regaddr`, and stores them in `buf`. Returns false on I2C problems.
